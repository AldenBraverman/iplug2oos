...
test::test(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new ITextControl(titleBounds, "test", IText(30)), kCtrlTagTitle);
...
#if IPLUG_EDITOR
void test::OnParentWindowResize(int width, int height)
{
  if(GetUI())
...
#if IPLUG_DSP
void test::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{ 
