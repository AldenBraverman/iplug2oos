...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\test Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\test Demo Installer.exe"
    ...
    files = [
      projectpath + "\\build-win\\test.vst3\\Contents\\x86_64-win\\test.vst3",
      projectpath + "\\build-win\\test_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\test-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\test-app_x64.pdb"  
  ]
