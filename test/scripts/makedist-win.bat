...
if %DEMO_ARG% == "demo" (
  echo Making test Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making test Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM msbuild test.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild test.sln /t:test-app;test-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\test.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\test.iss"
  REM goto END-is
